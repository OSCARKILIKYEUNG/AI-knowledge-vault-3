  ...
  try {
    ...
    if (!chatResp.ok) {
      ...
      return NextResponse.json(
        { ok: false, error: `openrouter_chat_failed: ${t}` },
        { status: 502 },
      );
    }
    ...
    // === 2) 產生 Embedding（把「標題 + 摘要 + 內容節錄 + 連結」串起來）
    const embedInput =
      [
        item.title ?? '',
        summary ?? '',
    ...
    if (!embResp.ok) {
      ...
      return NextResponse.json(
        { ok: false, error: `openrouter_embed_failed: ${t}` },
        { status: 502 },
      );
    }
    ...
    if (!Array.isArray(embedding) || embedding.length === 0) {
      return NextResponse.json(
        { ok: false, error: 'empty_embedding' },
        { status: 500 },
      );
    }

    // === 3) 回寫 DB：summary / embedding
